...
setup-release() {
  if [[ ! -f .env ]]; then
    curl -fsSL https://raw.githubusercontent.com/talex0909/azuracast-installer/main/sample.env -o .env
  fi
...
run-installer() {
  ...
  if [[ ! -f azuracast.env ]]; then
    curl -fsSL https://raw.githubusercontent.com/talex0909/azuracast-installer/main/azuracast.sample.env -o azuracast.env
  fi
  if [[ ! -f docker-compose.yml ]]; then
    curl -fsSL https://raw.githubusercontent.com/talex0909/azuracast-installer/main/docker-compose.sample.yml -o docker-compose.yml
  fi
  ...
  curl -fsSL https://raw.githubusercontent.com/talex0909/azuracast-installer/main/docker-compose.installer.yml -o docker-compose.installer.yml
...
update() {
  ...
  if ask "Are you ready to continue with the update?" Y; then
    ...
    curl -fsSL https://raw.githubusercontent.com/talex0909/azuracast-installer/main/docker.sh -o docker.new.sh
    ...
    if [[ ! -s docker-compose.new.yml ]]; then
      curl -fsSL https://raw.githubusercontent.com/talex0909/azuracast-installer/main/docker-compose.sample.yml -o docker-compose.new.yml
    fi
...
update-self() {
  ...
  curl -H 'Cache-Control: no-cache, no-store' -fsSL \
    https://raw.githubusercontent.com/talex0909/azuracast-installer/main/docker.sh?$(date +%s) \
    -o docker.sh
  chmod a+x docker.sh
